...
def get_response(user_query: str, db: SQLDatabase, chat_history: list):
    ...
    try:
        # Run the SQL query and get the response
        sql_response = db.run(sql_chain)
        response_content = sql_response
    except SQLAlchemyError as e:
        # Handle the SQL error and set a user-friendly message
        response_content = "There was an issue with the database query. Please check the query syntax or the data you are querying."
    ...
    chain = (
        RunnablePassthrough.assign(query=sql_chain).assign(
            ...
            response=lambda _: response_content,
        )
        | prompt
        | llm
        | StrOutputParser()
